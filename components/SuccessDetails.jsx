...
export default function SuccessDetails({ booking }) {
  ...
  const dt = booking?.dateISO
    ? new Date(booking.dateISO + 'T00:00:00')
    : (booking?.dateTime
  ...
  return (
    <div className="max-w-xl rounded-2xl shadow p-6 bg-white">
      ...
      <dl className="grid grid-cols-3 gap-2 text-sm">
        ...
        <dt className="font-medium">Product</dt><dd className="col-span-2">{booking.product || 'Good2Go Assessment'}</dd>
        <dt className="font-medium">Name</dt><dd className="col-span-2">{booking.clientName || booking.name || '—'}</dd>
        ...
        <dt className="font-medium">Amount</dt><dd className="col-span-2">{booking.amount ? `$${(booking.amount/100).toFixed(2)}` : '—'}</dd>
      </dl>

      <div className="mt-6 flex flex-wrap gap-3">
        <a className="px-4 py-2 rounded-xl bg-black text-white" href={`/api/ics?bid=${encodeURIComponent(booking.bookingRef || booking.bid || '')}`}>Add to Calendar (.ics)</a>
        <a className="px-4 py-2 rounded-xl border" target="_blank" rel="noopener noreferrer" href={`/success/receipt?bid=${encodeURIComponent(booking.bookingRef || booking.bid || '')}`}>Print receipt</a>
        <a className="px-4 py-2 rounded-xl border" href="/">Return to Home</a>
      </div>
    </div>
  );
}
