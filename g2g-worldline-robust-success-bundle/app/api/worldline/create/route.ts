...
/**
 * Robust WPRequest create:
 * - Accepts { productId } or { amountCents }
 * - Auto-creates booking if bookingId missing
 * - Forces return_url to /api/worldline/return?bid=<id> (ignores env/body to avoid misconfig)
 * - Sends a minimal, whitelist-safe field set
 * - Stores diagnostics: chosen endpoint/env/redirectUrl under bookings/<id>.worldline.create
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Always force our server return handler with the booking id
    const forcedReturnUrl = (() => {
      const origin = new URL(req.url).origin;
      const u = new URL("/api/worldline/return", origin);
      u.searchParams.set("bid", bid as string);
      return u.toString();
    })();

    // Whitelist-safe payload (no cancel_url etc.)
    ...
    // Extract HPP url (accepts :443)
    const urls = Array.from(raw.matchAll(/\bhttps?:\/\/[^\s<>"']+/gi)).map(m => m[0]);
    const redirectUrl = urls.find(u => /paymarkclick\.co\.nz(:443)?\/webpayments\/default\.aspx\?q=/i.test(u)) || urls.find(u => /paymarkclick\.co\.nz/i.test(u)) || null;
    if (!redirectUrl) {
      return NextResponse.json({ ok:false, status: res.status, endpoint, raw: raw.slice(0, 4000) }, { status: 502 });
    }
